...
func validate(input_buffer: Array, delta: float) -> bool:
	...
	# If there is an up-pressed input before an up-released input, activate.
	for input in input_buffer:
		if input.mode == 0 and input.action == "up":
			return false
		if input.mode == 1 and input.action == "up":
			return true
	
	return false

func activate(input_buffer: Array, delta: float):
	character.apply_movement(Vector2.UP)
